...
struct PositionedPattern {
	...
	PositionedPattern(int start_index_, int size_pattern_) : start_index(start_index_),
		size_pattern(size_pattern_) {};
};

struct Vertex {
	const size_t the_power_of_the_alphabet = 26;
	...
	bool terminal = false;
	std::vector<PositionedPattern> pattern_number;

	Vertex() : edge(the_power_of_the_alphabet, -1), transfer_through_i(the_power_of_the_alphabet, -1) {}
};


void AddPattern(std::vector<Vertex>& forest, size_t& size, const std::string& pattern,
	const int start_index, const int size_pattern) {
	size_t vertex_now = 0;
	for (size_t i = 0; i < pattern.size(); ++i) {
		size_t now_edge = pattern[i] - 'a';
...
int Processing(const std::string& pattern, std::vector<Vertex>& forest) {
	std::string s;
	size_t size = 1;
	...
	for (size_t i = 0; i < pattern.size(); ++i) {
		...
		if ((pattern[i] == '?' && !s.empty()) || (i == pattern.size() - 1 && pattern[i] != '?')) {
			AddPattern(forest, size, s, start_index, s.size());
			s = "";
		}
	}
	return count;
...
std::vector<int> AhoCorasick(const std::string& pattern, const std::string& text) {
	...
	size_t count = Processing(pattern, forest);
	//processing
	int now_vertex = 0;
